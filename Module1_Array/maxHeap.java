class BiTree{
    ...
    public Node ArrayToBiTree(int a[]){
        ...
        return Ntree[0];
        //return head to well linked tree for traversal
    }
    void printT(Node n){
        if(n==null) return;
        printT(n.l);
...
class ArrayToMaxHeapTree{
    ...
    ArrayToMaxHeapTree(int a[]){
        ...
        int deletionMaxHeap(){
            ...
            cbt[0]=cbt[size];
            cbt[size]=poppedElement;
            //<- put it back at freed index instead of discarding
            size--;
            //,_shrink heap boundary
            ...
            while(true){
                ...
                int rci=i*2+2;
                if(lci>size) break;
                int idxOfMaxBothChildren;
                if(rci>size){
                    idxOfMaxBothChildren=lci;
                } else{ 
                    idxOfMaxBothChildren=(cbt[lci]>cbt[rci])?lci:rci;
                }
                if(cbt[i]>=cbt[idxOfMaxBothChildren]) break;
                int temp=cbt[i];
                cbt[i]=cbt[idxOfMaxBothChildren];
                cbt[idxOfMaxBothChildren]=temp;
                i=idxOfMaxBothChildren;
            }
